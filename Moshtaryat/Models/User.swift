    ...
    class func registerUserWith(email: String, password: String, completion: @escaping (_ error: Error?) ->Void) {
        ...
        Auth.auth().createUser(withEmail: email, password: password) { (authDataResult, error) in
            ...
            if error == nil {
                ...
                //send email verification from firebase to the user when he enter worked email
...
func downloadUserFromFirestore(userId: String, email: String) {
    ...
    FirebaseReference(.User).document(userId).getDocument { (snapshot, error) in
        ...
        if snapshot.exists {
            ...
            //if we have a user save it loacally
            saveUserLocally(mUserDictionary: snapshot.data()! as NSDictionary)
        } else {
            //there is no user, so save new in firestore
